...
def validate_files(files):
    if not isinstance(files, list) or not files:
        raise ValueError("A non-empty files list is required.")
    validated = []
    for item in files:
        ...
        validated.append((path, content, size))
    return validated
...
def open_app():
    ...
    try:
        return jsonify({"ok": True, "message": open_allowlisted_app(app_name)})
...
def preview_files():
    data = request.get_json(silent=True) or {}
    try:
        validated = validate_files(data.get("files", []))
        preview = [
            {"path": str(path.relative_to(WORKSPACE)), "bytes": size}
            for path, _content, size in validated
        ]
...
def write_files():
    ...
    try:
        validated = validate_files(data.get("files", []))
        written = []
        for path, content, _size in validated:
